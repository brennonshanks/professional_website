...
3. The dispersion energy in molecular systems is often modeled by the Lennard-Jones potential, which gives the potential energy of two particles separated by a distance $r_{ij} = |\mathbf{r_i} - \mathbf{r_j}|$ Using Python, plot the Lennard-Jones potential and Lennard-Jones force (on separate plots) for a system with $\sigma = 1$, $\epsilon = 1$ for $0.9 \leq r \leq 4$. At what $r$ is the force zero? If two particles with a Lennard-Jones potential interaction are closer together than the radius of zero force, will they attract or repel each other?
